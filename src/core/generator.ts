...
interface GeneratorOptions {
  ...
  batchSize?: number; // Number of components to process before pausing
...
class Generator {
  ...
  async generateTests(analysisResults: AnalysisResult[]): Promise<GeneratedTest[]> {
    ...
    let allComponentsToTest: Array<{component: Component, filePath: string}> = [];
    
    for (const result of analysisResults) {
      // Filter components that need tests
      const componentsToTest = result.components.filter(component => component.needsTest);
      allComponentsToTest = allComponentsToTest.concat(
        componentsToTest.map(component => ({component, filePath: result.filePath}))
      );
    }
